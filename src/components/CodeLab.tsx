        ...
        bottom={
          <div className="flex flex-col bg-ink-900 xl:h-full xl:overflow-hidden">
            <LabToolbar
              ...
              onReset={() => setCode(spec.starter)}
            />
            <div className="px-4 py-3 xl:min-h-0 xl:flex-1 xl:overflow-y-auto">
              {outcome?.error && <p className="font-mono text-sm text-ember">{outcome.error}</p>}
              {passed && (
              ...
              {alreadySolved && !flash && (
                <p className="mt-2 font-mono text-[11px] text-paper/45">
                  Already in your solved set — Submit still re-runs tests.
                </p>
              )}
            </div>
          </div>
