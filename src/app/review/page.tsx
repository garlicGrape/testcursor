...
function ReviewList({ items, empty }: { items: ReviewItem[]; empty: string }) {
  ...
  return (
    <ul className="mt-3 divide-y divide-violet-500/15 rounded-2xl border border-violet-500/20 bg-ink-900/60">
      {items.map((item) => {
        const passed = item.record.localPass;
        return (
          <li key={item.problem.id}>
            ...
            >
              <span className={`font-mono text-xs ${passed ? "text-gold-400" : "text-ember"}`}>
                {passed ? "DUE" : "OPEN"}
              </span>
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <section className="mt-8">
        ...
        <ReviewList items={due} empty="No passed problems are due today. Keep the streak with a fresh solve." />
      </section>
      <section className="mt-8">
      ...
      <section className="mt-8">
        ...
        <ReviewList items={upcoming} empty="Nothing due in the next week." />
      </section>
    </div>
  );
}
