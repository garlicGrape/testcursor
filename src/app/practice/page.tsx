...
export default function PracticePage() {
  ...
  const [status, setStatus] = useState<"all" | "todo" | "done" | "due">("all");
  ...
  const dueIds = useMemo(
    () => new Set(reviewSchedule(progress).due.map((item) => item.problem.id)),
    [progress],
  );
  ...
  const rows = useMemo(() => {
    ...
    return PROBLEMS.filter((p) => {
      ...
      if (status === "due" && !dueIds.has(p.id)) return false;
      return true;
    });
  }, [difficulty, dueIds, kind, list, pattern, progress.solved, q, status]);
    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        ...
        <select
          className="rounded-md border border-violet-500/30 bg-ink-900 px-3 py-2 font-mono text-sm"
          value={status}
          onChange={(e) => setStatus(e.target.value as "all" | "todo" | "done" | "due")}
        >
      ...
      <ul className="mt-3 divide-y divide-violet-500/15 rounded-2xl border border-violet-500/20 bg-ink-900/60">
        ...
        {rows.map((p) => {
          const record = progress.solved[p.id];
          const done = Boolean(record?.localPass);
          ...
          return (
            <li key={p.id}>
              <Link href={`/practice/${p.id}`} className="flex flex-wrap items-center gap-3 px-4 py-3 hover:bg-ink-800/80">
                ...
                {due && (
                  <span className="rounded-full bg-ember/15 px-2 py-0.5 font-mono text-[10px] text-ember">due</span>
                )}
                {record && record.attempts > 0 && (
                  <span className="font-mono text-[11px] text-paper/35">
                    {record.attempts} try{record.attempts === 1 ? "" : "s"}
                  </span>
                )}
